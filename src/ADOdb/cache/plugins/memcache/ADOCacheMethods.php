...
final class ADOCacheMethods extends \ADOdb\cache\ADOCacheMethods
{
	...
	final protected function connect() : bool 
	{
		...
		*/
		if (class_exists('Memcache')) {
			...
			$message = 'MEMCACHE: The Memcache extension were found!';
		...
		*/
		...
		foreach($this->hosts as $host) {
			if (!@$memcache->addServer($host,$this->port)) {
				$failcnt += 1;
				if ($this->debug) {
					$message = sprintf("MEMCACHE: Attempt to add server %s on port %s to available memcache connections failed",  $host, $this->port);
					$this->connection->loggingObject->log(Logger::DEBUG,$message);
				}
			} else if ($this->debug) {
				sprintf("MEMCACHE: Added server %s on port %s to available memcache connection",  $host, $this->port);
				$this->connection->loggingObject->log(Logger::DEBUG,$message);
			}
		}
		
		if ($failcnt == $failTarget) {
			$message = 'MEMCACHE: Can\'t connect to any memcache server';
			$this->connection->loggingObject->log(Logger::CRITICAL,$message);
			return false;
		}
		/*
		...
		*/
		...
		if (is_array($this->memCacheOptions) && $this->debug) {
			$message = 'MEMCACHE: Defined memcache options are ignored for the Memcache library';
			$this->connection->loggingObject->log(Logger::DEBUG,$message);
		}
	...
	*/
	final protected	function flushall() : void
	{
		...
		if (!$del) {
			$message = 'MEMCACHE: flushall in MemCache service failed';
			$this->connection->loggingObject->log(Logger::CRITICAL,$message);
		} else if ($this->debug) {
			$message = 'MEMCACHE: flushall in MemCache service succeeded';
			$this->connection->loggingObject->log(Logger::DEBUG,$message);
		}
	}
	...
	*/
	final protected function flushcache(
		...
		{
			if (!$del) 
				$message = "MEMCACHE: $filename entry doesn't exist on memcache server";
			else 
				$message = "MEMCACHE: $filename entry flushed from memcache server";
	...
	*/
	final protected	function readcache(
		...
		if (!$rs) {
			$message = sprintf('MEMCACHE: Item with key %s doesn\'t exist on the memcache server.', $filename);
			$this->connection->loggingObject->log(Logger::CRITICAL,$message);
			return null;
		} else if ($this->debug) {
			$message = sprintf('MEMCACHE: Item with key %s retrieved from the memcache server.', $filename);
		...
		if (! is_object($rs)) {
			$message = 'MEMCACHE: Unable to unserialize $rs';
			$this->connection->loggingObject->log(Logger::CRITICAL,$message);
			return null;
		}
		if ($rs->timeCreated == 0)
		{	
			if ($this->debug)
			{
				$message = 'MEMCACHE: Time on recordset is zero';
				$this->connection->loggingObject->log(Logger::DEBUG,$message);
			}
		...
		if ($tdiff <= 2) {
			switch($tdiff) {
				...
				case 1:
				if ((rand() & 3) == 0) {
					$message = "MEMCACHE: Timeout 1";
					$this->connection->loggingObject->log(Logger::CRITICAL,$message);
				}
				break;
				default:
				$message = "MEMCACHE: Timeout 0";
				$this->connection->loggingObject->log(Logger::CRITICAL,$message);
				return null;
			}
		}
	...
	*/
	final protected	function writecache(
		...
		/*
		* Windows connection module
		*/
		if (!$this->cacheLibrary->set($filename, $contents, $this->compress ? MEMCACHE_COMPRESSED : 0, $secs2cache)) {
			$failed=true;
		}
		
		if($failed) {
			$message = 'MEMCACHE: Failed to save data at the memcache server';
			$this->connection->loggingObject->log(Logger::CRITICAL,$message);
			return false;
		
		} else if ($this->debug) {
		
			$message = sprintf('MEMCACHE: Successfully wrote query contents on key %s to memcache server',$filename);
